...
const getPlaceCoords  = async ( city, headersSettings = getPlaceCoordsHeaders) =>{
    ...
    const resp = await fetch(`https://trouverunlogement.lescrous.fr/photon/api?q=${city}&limit=18&lang=fr`, { method: "GET", headersSettings})
    ...
    if(returnData.features[0]){
        ...
        coordsArrayParsed = [{lon:coordsArray[0], lat:coordsArray[1]}, {lon:coordsArray[2], lat:coordsArray[3]}] || []
    }
